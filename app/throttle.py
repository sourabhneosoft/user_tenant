...
class TenantThrottle(BaseThrottle):
    ...
    def allow_request(self, request, view):
        ...
        todays_requests = ThrottleRequest.objects.filter(tenant=tenant, requested_on__date=datetime.datetime.today())
        is_allowed = True

        if len(todays_requests) > 50 and todays_requests.filter(
                requested_on__gte=datetime.datetime.now() - datetime.timedelta(seconds=10)):
            is_allowed = False
